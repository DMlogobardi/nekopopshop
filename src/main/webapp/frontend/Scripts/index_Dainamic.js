...
document.addEventListener("DOMContentLoaded", function () {
    ...
    fetch("getcatalog?dayProduct=ok&productType=prod")
        .then(response => {
            if (response.status === 500){
                throw new Error("Errore nel caricamento del prodotto");
            }
        ...
        .then(data => {
            console.log(data)
            const container = document.getElementById("dayOfferProd");
            let imageSrc;

            if(data[0].imgProd)
                imageSrc = data[0].imgProd.startsWith("data:")
                    ? data.imgProd
                    : `data:image/jpeg;base64,${data[0].imgProd}`;

            let prezzoOriginale = data[0].prezzo;
            let prezzoAumentato = (data[0].prezzo * 1.3).toFixed(2);
            ...
            container.innerHTML = `
                            ...
                            <div class="flex flex-col md:flex-row items-center gap-4">
                                ...
                                <div class="md:flex-1 flex flex-col md:flex-row items-center gap-4">
                                    ...
                                    <div class="text-center md:text-left">
                                        <h3 class="font-bold text-xl text-nekopurple mb-1">${data[0].nome}</h3>
                                        <div class="flex justify-center md:justify-start items-center gap-1">
                                            <p class="text-gray-500 line-through text-sm">&#8364 ${prezzoAumentato}</p>
                                            <p class="text-2xl font-bold text-nekopink">&#8364 ${data[0].prezzo}</p>
                                        </div>
                                        <div class="mt-2 flex gap-2 justify-center md:justify-start">
                                            <span class="bg-nekoorange px-2 py-1 rounded-lg text-white text-xs">super sconto</span>
                                            <span class="bg-nekored px-2 py-1 rounded-lg text-white text-xs">-30%</span>
...
document.addEventListener("DOMContentLoaded", function () {
    fetch("getcatalog?dayProduct=figure&productType=prod&limit=1")
        .then(response => {
            if (response.status === 500){
                throw new Error("Errore nel caricamento del prodotto");
            }
...
document.addEventListener("DOMContentLoaded", function () {
    fetch("getcatalog?limit=3&productType=prod")
        .then(response => {
            if (response.status === 500) {
                throw new Error("Errore nel caricamento del prodotto");
            }
...
document.addEventListener("DOMContentLoaded", function () {
    fetch("getcatalog?limit=10&productType=vol")
        .then(response => {
