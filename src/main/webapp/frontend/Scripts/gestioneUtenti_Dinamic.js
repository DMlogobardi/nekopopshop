...
async function caricaClienti(pag = 1){
    const params = new URLSearchParams();
    ...
    try {
        ...
        mostraErrore("Errore nell caricamento");
        console.error('Errore nella fetch:', error);
    }
}
...
window.initUtent = async function() {
    await utentiRegistrati();
    await caricaClienti(curentPage);

    document.getElementById("prec").addEventListener("click",  async function () {
        if(curentPage > 1){
            curentPage -= 1;
            await caricaClienti(curentPage);
        } else {
    ...
    document.getElementById("suc").addEventListener("click",  async function (){
        if(elem){
            curentPage += 1;
            await caricaClienti(curentPage);
        }
