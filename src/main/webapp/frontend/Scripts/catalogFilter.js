...
function caricaVolumiTag(pag = 0, tag) {
    console.log("tag: " + tag);
    fetch(`getcatalog?page=${pag}&limit=3&productType=vol&filter=null&categoria=${tag}`)
        .then(res => res.json())
        .then(data => {
            const prodotti = data.filter(item => item.idVolume === undefined);  // prodotti
            const volumi = data.filter(item => item.idVolume !== undefined);    // volumi
...
function aggiornaPaginazioneVolTag(totalCount, currentPage, perPage, onPageChange, tag) {
    const totalPages = Math.ceil(totalCount / perPage);
    const pagination = document.getElementById('pagination1');
    const prevBtn = document.getElementById('prev-btn1');
    const nextBtn = document.getElementById('next-btn1');
    ...
    function createPageButton(label, page, isActive = false, isEllipsis = false) {
        ...
        if (isEllipsis) {
            ...
            el.addEventListener('click', (e) => {
                e.preventDefault();
                if (page !== currentPage) onPageChange(page, tag);
            });
        }
    ...
    prevBtn.onclick = () => {
        if (currentPage > 1) onPageChange(currentPage - 1, tag);
    };

    nextBtn.onclick = () => {
        if (currentPage < totalPages) onPageChange(currentPage + 1, tag);
    };
}